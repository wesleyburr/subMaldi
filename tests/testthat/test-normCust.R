...
test_that("No norm_mz selected when normMethod_custom yields error", {
  expect_error(normSpectra(dat = Master2, mass_dat = "full_mz", method = "custom", 
                           spec1 = "Before1"))
})

out <- normSpectra(Master2, "full_mz", method = "custom",
...
out <- normSpectra(Master, "full_mz", method = "custom_imprecise", 
